...
function fetch(languages, action) {

  var limit = languages.get('paginator').get('limitPerPage');
  var total = languages.get('paginator').get('totalPage');
  var page = languages.get('paginator').get('currentPage');
  var term = languages.get('searchTerm');
  var search = true;
  var allowNavigation = {
    next: true,
    prev: false
    ...
    if (search) {
      dispatch(_defineProperty({}, _api.CHAIN_API, [function () {
        return _defineProperty({}, _api.CALL_API, {
          method: 'get',
          type: 'external',
          path: '/language/search?limit=' + limit + '&page=' + page + '&name=' + term,
...
function create_new(language) {
  return function (dispatch, getState) {
    var newRow = getState().ListLanguages.get('newRow');
...
function remove(language) {
  return function (dispatch, getState) {
    var selectedRow = getState().ListLanguages.get('selectedRow');
    dispatch(is_saving(true));
    dispatch(_defineProperty({}, _api.CHAIN_API, [function () {
      return _defineProperty({}, _api.CALL_API, {
        method: 'delete',
