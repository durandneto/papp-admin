...
export function fetch(languages, action) {

  let limit = languages.get('paginator').get('limitPerPage')
  let total = languages.get('paginator').get('totalPage')
  let page = languages.get('paginator').get('currentPage')
  let term = languages.get('searchTerm')
  let search = true
  let allowNavigation = {
      next : true
      , prev : false
            ...
            ()=> {
              return {
                [CALL_API]: {
                  method: 'get',
                  type : 'external',
                  path: '/language/search?limit=' + limit
                  + '&page='+ page
                  + '&name='+ term,
...
export function create_new(language) {
  return (dispatch, getState) => {
    let newRow = getState().ListLanguages.get('newRow')
...
export function remove(language) {
  return (dispatch, getState) => {
    let selectedRow = getState().ListLanguages.get('selectedRow')
    dispatch(is_saving(true))
    dispatch({
      [CHAIN_API]: [
          ()=> {
            return {
              [CALL_API]: {
                method: 'delete',
                type : 'external',
                path: '/language/remove/'+selectedRow.get('id'),
...
export function update(language) {
  return (dispatch, getState) => {
    let selectedRow = getState().ListLanguages.get('selectedRow')
    dispatch(is_saving(true))
    dispatch({
